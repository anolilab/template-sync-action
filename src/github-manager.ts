...
import { IGithubManager, IGithubManagerBranch, IGithubManagerPulls, IGithubManagerRepos } from "./interfaces";
...
export class GithubManager implements IGithubManager {
        ...
        return {
            ...
            has: async (owner: string, repo: string, branch: string) => {
                try {
                    ...
                    throw new Error(`Failed to check if branch ${branch} exist; ${inspect(error)}`);
                }
            },
        };
    }
    ...
    get repos(): IGithubManagerRepos {
        const getArchiveLink = async (owner: string, repo: string, ref: string): Promise<Buffer> => {
        ...
        return {
            ...
            getArchiveLink,
            downloadRepository: async (owner: string, repo: string, ref: string): Promise<void> => {
